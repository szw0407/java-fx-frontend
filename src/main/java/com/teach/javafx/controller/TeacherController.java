...
public class TeacherController extends ToolController {
    ...
    private boolean validateInput() {
        ...
        // 必填字段验证
        if (numField.getText().isBlank()) errors.append("学号不能为空\n");
        if (nameField.getText().isBlank()) errors.append("姓名不能为空\n");

        // 格式验证
        if (!numField.getText().matches("\\d{10}"))
            errors.append("学号必须是10位数字\n");
        if (!emailField.getText().matches("^\\w+([.-]?\\w+)*@\\w+([.-]?\\w+)*(.\\w{2,3})+$"))
    ...
    protected void onAddButtonClick() {
        ...
        form.put("birthday", birthdayPick.getValue().toString());
